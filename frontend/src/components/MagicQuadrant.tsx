...
type ValueTooltipProps = {
  active?: boolean;
  payload?: Array<{ payload: StockData }>;
};
...
export function MagicQuadrant({ data }: { data: StockData[] }) {
  const validData = data.filter(d =>
    d["Margin of Safety"] !== null && d.ROIC !== null
    && (d["Margin of Safety"] as number) >= -300
    && (d["Margin of Safety"] as number) <= 100
  );
  const omitted = data.filter(d => d["Margin of Safety"] !== null && d.ROIC !== null).length - validData.length;
  ...
  return (
    <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 h-[400px] flex flex-col relative w-full min-w-0 overflow-hidden shadow-[0_4px_20px_-4px_rgba(0,0,0,0.5)] backdrop-blur-md">
      ...
      {omitted > 0 && <p className="text-[11px] text-slate-500 -mt-2 mb-2">{omitted} extreme negative outlier{omitted === 1 ? "" : "s"} omitted below −300% for readability.</p>}
        ...
        <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0} initialDimension={{ width: 600, height: 300 }}>
          <ScatterChart margin={{ top: 10, right: 20, bottom: 20, left: -20 }}>
            ...
            <YAxis 
              ...
              tickLine={false}
            />
            <Tooltip content={<ValueTooltip />} cursor={{ strokeDasharray: '3 3', stroke: '#475569' }} />
